...
use jet_initialization,        only : register_jet_OBC, jet_OBC_end, jet_OBC_CS
...
type, public :: update_OBC_CS ; private
  ...
  logical :: use_jet = .false.          !< If true, use the jet open boundary.
  ...
  type(jet_OBC_CS), pointer :: jet_OBC_CSp => NULL()
...
# include "version_variable.h"
  ...
  call get_param(param_file, mdl, "USE_TIDAL_BAY_OBC", CS%use_tidal_bay, &
                 ...
                 default=.false.)
  call get_param(param_file, mdl, "USE_JET_OBC", CS%use_jet, &
                 "If true, use the jet open boundary.", &
                 default=.false.)
  ...
  call get_param(param_file, mdl, "OBC_USER_CONFIG", config, &
               ...
               "   jet - asymmetric jet forcing on the western boundary\n"//&
...
!  elseif (trim(config) == "dyed_channel") then
  ...
  if (CS%use_tidal_bay) CS%use_tidal_bay = &
    register_tidal_bay_OBC(param_file, CS%tidal_bay_OBC, US, &
               OBC%OBC_Reg)
  if (CS%use_jet) CS%use_jet = &
    register_jet_OBC(param_file, CS%jet_OBC_CSp, US, &
               OBC%OBC_Reg)
...
subroutine OBC_register_end(CS)
  ...
  if (CS%use_jet) call jet_OBC_end(CS%jet_OBC_CSp)
